...
description: |
  Jako první na vás v Search Consoli vyskočí graf výkonu webu v čase. Časové řady ze Seaerch Console jdou ale vizualizovat o dost lépe a jdou z nich pak vypozorovat zajímavé věci.
author:
  - name: Marek Prokop
...
Na druhou stranu má článek i čistě metodickou, na programovacím jazyce nezávislou rovinu. Některé uvedené časové řady a jejich vizualizace jdou vytvořit i jinak, např. v Data Studiu (na příklad odkazuju dál) nebo v Google Sheets. I bez znalosti R jde tedy článek využít jako vzorník výstupů, které jdou z dat Search Console jakkoli udělat a které dávají určitý smysl.

Všechny ukázky čerpají z dat webu [Glamour Cabaret](https://www.glamourcabaret.cz/) Alžběty Faltysové. Jako hobby projekt bez větších investic sice neohromí gigantickými čísly, ale zároveň hezky ukazuje některé vzorce typické pro přirozeně a dobře budovaný obsah se zajímavým potenciálem na další zlepšování. Alžbětě moc děkuju, že mi data pro článek poskytla.
...
Jako vždy *tidyverse*, *searchConsoleR* pro práci se Search Consolí, *lubridate* pro práci s daty a časem a pro stejný účel alternativně i *tsibble*. Balíček *feasts* je zde jen pro dekompozici časové řady na trend, sezónnost a šum, a balíček *slider* umožňuje výpočet klouzavých průměrů.

Dlužno dodat, že specializované balíčky zde používám jen v nejnutnější míře a zatím mi to tak vyhovuje. Na druhou stranu přiznávám, že některé věci by šly řešit elegantněji plným nasazením specializovaných balíčků typu [timetk](https://business-science.github.io/timetk/index.html), [tsbox](https://www.tsbox.help/reference/index.html), [tsibble](https://tsibble.tidyverts.org/index.html) apod.
...
```{r}
period_end <- floor_date(today() - 3, unit = "week")
...
sc_plot <- function(
  ...
  scale_x_fun <- scale_x_date
  if (inherits(df[[1]], "yearweek")) {
    scale_x_fun <- scale_x_yearweek
  }
  if (inherits(df[[1]], "yearmonth")) {
    scale_x_fun <- scale_x_yearmonth
  }
...
První funkce tranformuje datum (vektor) na delší období (týden, měsíc, čtvrtletí). Použije k tomu funkce a třídy z balíčku *tsibble*.

```{r}
trans_date <- function(x, per = "month") {
  if (per == "week") {
    tsibble::yearweek(x)
  } else if (per == "month") {
    tsibble::yearmonth(x)
    ...
    stop("Argument per has to be either 'week', 'month', or 'quarter'.")
  }
}
```

Druhá funkce agreguje tabulku základních metrik.

```{r}
sc_aggregate <- function(df, per = "week") {
  df |>
    group_by("{per}" := trans_date(date, per)) |> 
    summarise(
...
Třetí funkce znovu spočítá počty unikátních stránek či dotazů. Předpokládá, že vstupní tabulka obsahuje sloupce `date` a sloupec řetězců, keré se mají spočítat. Na dalších sloupcích nezáleží.
...
sc_aggregate_all_metrics("week") |> 
  filter(week >= yearweek(as.Date("2021-08-01"))) |>
  sc_reshape() |>
...
sc_aggregate_all_metrics("month") |> 
  filter(
    month >= yearmonth(as.Date("2021-08-01")),
    month <= yearmonth(as.Date("2022-03-01"))
  ) |>
  sc_reshape() |>
  sc_plot("month")
```
...
```{r fig.asp=1, layout="l-body-outset"}
sc_all_metrics_by_date |>
  mutate(week = yearweek(date), .before = date) |> 
  filter(week >= yearweek(as.Date("2021-08-01"))) |> 
  select(!date) |> 
  sc_reshape() |> 
...
sc_aggregate_all_metrics("week") |> 
  ...
  filter(week >= yearweek(as.Date("2022-01-03"))) |> 
...
Zde je příklad posledních 12 týdnů v porovnání s předešlými 12 týdny vykreslený po dnech. Ukazuje, o kolik byla daná metrika v daný den vyšší než před 12 tdny.
